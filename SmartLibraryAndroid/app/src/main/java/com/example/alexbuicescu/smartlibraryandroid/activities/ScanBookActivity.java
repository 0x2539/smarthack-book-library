...
import com.example.alexbuicescu.smartlibraryandroid.pojos.eventbus.NFCMessage;
import com.example.alexbuicescu.smartlibraryandroid.rest.RestClient;
import com.example.alexbuicescu.smartlibraryandroid.utils.NdefReaderTask;
...
public class ScanBookActivity extends BaseActivity {
    ...
    @Subscribe
    public void onEvent(NFCMessage message) {
        Log.i(TAG, "onEvent NFCMessage: " + message.isSuccess());
        if (message.isSuccess()) {
            try {
                long bookId = Long.parseLong(message.getNfcValue());
                Intent intent = new Intent(ScanBookActivity.this, BookDetailsActivity.class);
                intent.putExtra(BookDetailsActivity.KEY_BOOK_ID, bookId);
                ScanBookActivity.this.startActivity(intent);
                finish();
                return;
            }
            catch (Exception e) {
                Toast.makeText(ScanBookActivity.this, "Invalid book", Toast.LENGTH_SHORT).show();
            }
            Log.i(TAG, "onEvent: " + message.getNfcValue());
        }
