 ...
 */
public class SearchCallback extends HttpCallbackCustom<EmptyRequest, ArrayList<MainBooksResponse>> {

    private final String TAG = SearchCallback.class.getSimpleName();

    public SearchCallback(Context context, EmptyRequest request) {
    ...
    public void onResponse(Call<ArrayList<MainBooksResponse>> call, Response<ArrayList<MainBooksResponse>> response) {
        ...
        if (onResponseBase(response)) {
            ...
            BooksManager.getInstance().setSearchedResultBooks(response.body());
            EventBus.getDefault().post(new SearchMessage(true));
        } else {
            Log.i(TAG, "onResponse: sending false");
            EventBus.getDefault().post(new SearchMessage(false));
        }
    }
    ...
    private SearchMessage getEventBusMessage() {
